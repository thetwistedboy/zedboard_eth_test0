...
ring_buf_s ethcli_tx_buf = {.buf = {'\0'}, .head = ethcli_tx_buf.buf, .tail = ethcli_tx_buf.buf};
ring_buf_s ethcli_rx_buf = {.buf = {'\0'}, .head = ethcli_rx_buf.buf, .tail = ethcli_rx_buf.buf};
...
err_t tcp_recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err){
    ...
    char *dataptr = p->payload;
    for(int i = 0; i < p->len; i++){
        *ethcli_rx_buf.head = dataptr[i];
        if(ethcli_rx_buf.head < (ethcli_rx_buf.buf + 1024)){
            ethcli_rx_buf.head++;
        } else{
            ethcli_rx_buf.head = ethcli_rx_buf.buf;
        }
    }
...
void eth_putchar(char c){
    ...
    static u16_t length = 0;
    XTime_GetTime(&tstart);
    
    *ethcli_tx_buf.head = c;
    ethcli_tx_buf.head++;
    length++;
    
    if((tstart - tend > 10000) | (length == 1024)){
        tcp_write(pcb, ethcli_tx_buf.buf, length, TCP_WRITE_FLAG_COPY);
        ethcli_tx_buf.head = ethcli_tx_buf.buf;
        length = 0;
    }
...
char eth_getchar(void){
    char c = *ethcli_rx_buf.tail;
    
    if(ethcli_rx_buf.tail != ethcli_rx_buf.head){
        if(ethcli_rx_buf.tail < (ethcli_rx_buf.buf + 1024)){
            ethcli_rx_buf.tail++;
        } else{
            ethcli_rx_buf.tail = ethcli_rx_buf.buf;
        }
    }
